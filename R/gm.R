...
#' @param do_simultaneous If TRUE, compute all the mirror stats simultaneously as in alg 2 of the neural gaussian mirror paper.
...
gm  = function(y, x, q=0.1, family="gaussian", ncores = 1,rep.boot=100, var.est=FALSE, do_simultaneous = FALSE){
    # Basics
    n = length(y)
    p = dim(x)[2]
    x= scale(x)
    # Determine active set -- we are using the post-selection version of th GM.
    ...
    # Compute scale params for mirror noise
    get_mirror_noise_scale = function(j){
      ...
      if(pp<0.05){
        xnj.idx=1:10
        pxnj = xnj.svd$u[, xnj.idx]
        sigmaz = (1 - t(xj) %*% pxnj  %*%  solve(t(pxnj) %*% pxnj) %*% t(pxnj) %*% xj/n)/(1- sum(coef_ast!=0)/n)
      }else{
        sigmaz = pp/(1- sum(coef_ast!=0)/n)
      }
      return(sigmaz)
    }
    sigmaz_vec = unlist(parallel::mclapply(1:p, get_mirror_noise_scale, mc.cores = ncores))
    ...
    if(do_simultaneous){
      if(var.est==TRUE){
        stop("Sorry, variance estimation is not yet implemented when `do_simultaneous==TRUE`.")
      }
      xnew = cbind(x + z, x - z)
      fit1.lasso = glmnet(xnew, y, family=family, lambda=lambda_ast)
      b11 = coef(fit1.lasso, s = 'lambda.min')[2:(ncol(z)+1)]
      b12 = coef(fit1.lasso, s = 'lambda.min')[(ncol(z)+2):(2*ncol(z)+1)]
      return(list(gm_statistics = abs(b11+b12)- abs(b11-b12)))
    }
    w_vec = numeric(p)
