#' Generate noise variable for mirrored feature construction
#'
...
#' @return z: matrix containing noise variables Z at the right scale for constructing mirrors x + z, x - z
#'
#' @export
#' @import glmnet
#'
generate_mirrors = function(y, x, family, ncores, nlambda){
  n = length(y)
  p = dim(x)[2]
  ...
  para_list = foreach(j = 1:p,.packages='glmnet') %dopar% {
  ...
  stopCluster(cl)
  return(list(z=z, sigmaz_vec=sigmaz_vec, lambda_ast=lambda_ast))
}
...
#' @param nlambda The number of lambda values. The default is 100.
#' @param do_simultaneous Mirror all variables simultaneously?
...
#'
gm = function(y, x, q=0.1, family="gaussian", ncores = 1,rep.boot=100, var.est=FALSE, do_simultaneous = TRUE){
  n = length(y)
  p = dim(x)[2]
  x= scale(x)
  mirror_noise = generate_mirrors(y, x, family, ncores, nlambda)
  z = mirror_noise$z
  sigmaz_vec = mirror_noise$sigmaz_vec
  lambda_ast = mirror_noise$lambda_ast
  rm(mirror_noise)

  w_vec = numeric(p)
  # Compute mirror statistics all at once
  if(do_simultaneous){
    xnew = cbind(x + z, x - z)
    fit1.lasso = glmnet(xnew, y, family=family, lambda=lambda_ast)
    b11 = coef(fit1.lasso, s = 'lambda.min')[  2:(ncol(x)+1) ]
    b12 = coef(fit1.lasso, s = 'lambda.min')[-(1:(ncol(x)+1))]
    w =  abs( b11 + b12 ) - abs( b11 - b12 )
    if(var.est==TRUE){
      stop("Only implemented when `do_simultaneous` is FALSE")
    }
    return(list(gm_statistics = w))
  }
  # Compute mirror statistics one at a time
  cl <- makeCluster(ncores)
  registerDoParallel(cl)
